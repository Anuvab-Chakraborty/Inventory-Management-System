...
class productClass:
    ...
    #=================================
        ...
        # ====title=====
        title = Label(product_Frame, text="Manage Products Details", font=("goudy old style", 15), bg="#0f4d7d", fg="white").pack(side=TOP,fill=X)
        ...
        # =====SearchFrame=======
        SearchFrame = LabelFrame(self.root, text="Search Employee", font=("goudy new style", 12, "bold"), bd=2,relief=RIDGE, bg="white")
...
# ===================================================
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="" or self.var_price.get()=="":
                messagebox.showerror("Error", "ALL fields are required", parent=self.root)
            else:
                cur.execute("Select * from product where name=?", (self.var_name.get(),))
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "This product is already present, Try differnt",parent=self.root)
                else:
        ...
        try:
            if self.var_pid.get() == "":
                ...
                if row == None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success", "Procuct Updated Successfully", parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to : {str(ex)}", parent=self.root)
